...
var Card = {
	add: function (key,arr,place)
    {
		...
		if(!found||arr[key]==undefined){
			if(place){
				...
				if(arr.length==0){ place[key]=1; }
				console.log(place[key])
				if(place[key]==undefined){place[key]=last+1;}
			}
        	return arr.push(key)
		}
		debuger.info("Нашел элемент "+key)
    },
    addFirst: function (key,arr){
		var found = arr.some(function (el) {
			return el === key;
		});
		if(!found){
	...
	remove: function (key,arr,place)
    {
    	var idx = arr.indexOf(key);
    	// var idp = Object.keys(place).indexOf(key);
	    if (idx != -1) {
	    	place[key]=undefined
